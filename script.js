...
function addMinRow() {
    ...
    el.minutosCont.insertAdjacentHTML(
      ...
      <button class="btn btn-ghost" type="button" aria-label="Eliminar" title="Eliminar" onclick="removeMinRow('${rowId}')">🗑️</button>
    </div>`
    );
}
...
function resourceCard({ icon, title, desc, href, btnText, accent }) {
    ...
    <a href="${safeHref}" target="_blank" rel="noopener" style="display:inline-block;margin-top:14px;padding:10px 18px;background:${accent};color:white;border-radius:10px;text-decoration:none;font-weight:900;">
      ${b}
    </a>
  </div>`;
}
...
function actionButton(href, text) {
    ...
    <a href="${safeHref}" target="_blank" rel="noopener" style="display:inline-block;padding:12px 26px;background:#0a4aa6;color:white;text-decoration:none;font-weight:900;border-radius:12px;box-shadow:0 0 10px rgba(0,0,0,0.18);">
      ${t}
    </a>
  </div>`;
}
...
<script>
(function(){function r(){if(window.MathJax&&window.MathJax.typesetPromise){window.MathJax.typesetPromise();}}
...
function generarHTML() {
  ...
  `;
    ...
    const videoBlock = linkWatch
        ...
        <div style="display:flex;gap:12px;align-items:flex-start;">
          ...
          <div style="flex:1;">
            ...
            <a href="${linkWatch}" target="_blank" rel="noopener" style="display:inline-block;padding:10px 16px;background:#0a4aa6;color:white;text-decoration:none;font-weight:900;border-radius:12px;">
    ...
    const inner = [header, videoBlock, minutosBlock, transcripcionBlock, profBlock, actividadBlock].filter(Boolean).join("");
